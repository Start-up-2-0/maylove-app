  ...
  <nav class="wiz-stepper" aria-label="Etapas">
    ...
    <div class="wizard-stepper-scroll flex gap-2 pb-1 mt-3">
      ...
      >
        ...
        {{ BOUQUET_WIZARD_STEP_LABELS[step] }}
      </button>
    </div>
  </nav>
</template>
...
import {
  BOUQUET_WIZARD_STEP_LABELS,
  BOUQUET_WIZARD_STEPS,
  type BouquetWizardStep,
} from './bouquetWizardSteps'

const props = withDefaults(
  defineProps<{
    currentStep: BouquetWizardStep
    steps?: BouquetWizardStep[]
  }>(),
  { steps: () => [...BOUQUET_WIZARD_STEPS] },
)

defineEmits<{ go: [step: BouquetWizardStep] }>()

<template>
  <div class="view gal-dash ml-fade-up">
    <header class="gal-dash__head">
      ...
      <RouterLink
        v-if="loading || error || rawAlbums.length > 0"
    ...
    <div v-if="!loading && !error && rawAlbums.length" class="dash-filters">
      <label class="dash-search"><span class="sr-only">Buscar álbuns</span><input v-model.trim="search" type="search" placeholder="Buscar por título ou descrição" /></label>
    ...
    <div v-else-if="filteredAlbums.length === 0" class="ml-card empty">
      <span class="empty__glyph" aria-hidden="true">🔎</span>
      <h2>Nenhum álbum neste filtro</h2>
...
const filteredAlbums = computed(() => {
  ...
  const result = rawAlbums.value.filter((album) => {
    const status = activeFilter.value === 'all' || (activeFilter.value === 'published' ? album.status === 'published' : album.status !== 'published')
    const text = [album.title, album.subtitle, album.slug].some((value) => value?.toLocaleLowerCase('pt-BR').includes(term))
